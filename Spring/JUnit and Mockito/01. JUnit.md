...
public class UserServiceTests {
	...
	void testDemo() {
		...
		assertEquals(expected, actual, () -> expected + " not equals to "+actual);
...
when we write alot of test cases, essesntially they run in a random order, and each test method has a new object of the test class
JUnit also provides us with lifecycle methods, helps us execute our test at specific phases 
the lifecycle methods have different kind of annotations
1. @BeforeAll
...
# the test methods can not be private because junit uses reflection to invoke them
...
the source method needs to be static and needs to return a Stream of arguments
...
public class CalculatorTest{
	...
	public static Stream<Arguments> IntegerDivisionTestController(){
		return Stream.of(
...
public class CalculatorTest{
	...
	public static Stream<Arguments> testIntegerDivision(){
		return Stream.of(
			Arguments.of(33,1,33),
			Arguments.of(13,1,13)
		);
